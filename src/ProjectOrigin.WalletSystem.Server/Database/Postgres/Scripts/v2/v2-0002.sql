...
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'old_deposit_endpoints') THEN
    ...
    INSERT INTO receive_endpoints (id, wallet_id, wallet_position, public_key, reference_text)
    SELECT old.Id, old.WalletId, old.WalletPosition, old.PublicKey, old.ReferenceText
    FROM old_deposit_endpoints AS old
...
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'old_certificates') THEN
    ...
    SELECT old.Id, registries.Name, old.StartDate, old.EndDate, old.GridArea, old.CertificateType
    FROM old_certificates AS old;
    INNER JOIN registries ON old.RegistryId = registries.id;
END IF;
...
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'old_attributes') THEN
    ...
    SELECT old.Id, old.KeyAtr, old.ValueAtr, old.CertifcateId, registries.Name
    FROM old_attributes AS old
    INNER JOIN registries ON old.RegistryId = registries.id;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'old_slices') THEN
    INSERT INTO slices (id, receive_endpoint_id, receive_endpoint_position, slice_state, registry_name, certificate_id, quantity, random_r)
    SELECT old.Id, old.DepositEndpointId, old.DepositEndpointPosition, old.SliceState, registries.Name, old.CertificateId, old.Quantity, old.RandomR
    FROM old_slices AS old
    INNER JOIN registries ON old.RegistryId = registries.id;
    INNER JOIN DepositEndpoints ON old.DepositEndpointId = DepositEndpoints.id
    WHERE
        DepositEndpoints.WalletId IS NOT NULL;

    INSERT INTO sent_slices (id, deposit_endpoint_id, deposit_endpoint_position, slice_state, registry_name, certificate_id, quantity, random_r)
    SELECT old.Id, old.DepositEndpointId, old.DepositEndpointPosition, old.SliceState, registries.Name, old.CertificateId, old.Quantity, old.RandomR
    FROM old_slices AS old
    INNER JOIN registries ON old.RegistryId = registries.id;
    INNER JOIN DepositEndpoints ON old.DepositEndpointId = DepositEndpoints.id
    WHERE
        DepositEndpoints.WalletId IS NULL;
END IF;
...
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'old_claims') THEN
    ...
    SELECT old.Id, old.ProductionSliceId, old.ConsumptionSliceId, old.State
